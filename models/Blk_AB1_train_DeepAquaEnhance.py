...
from models.b1ab1_MuLA_GAN_Initial_model_1  import MuLA_GAN_Generator, Discriminator
...
checkpoint_dir = os.path.join("bkab1/", dataset_name)
...
for epoch in range(epoch, num_epochs):
    for i, batch in enumerate(dataloader):
        ...
        if imgs_fake.size() != imgs_good_gt.size():
            
            #print(f"Size mismatch: imgs_fake {imgs_fake.size()}, imgs_good_gt {imgs_good_gt.size()}")
            imgs_fake = F.interpolate(imgs_fake, size=imgs_good_gt.size()[2:])
    ...
    if (epoch % ckpt_interval == 0):
        torch.save(generator.state_dict(), "bkab1/%s/generator_%d.pth" % (dataset_name, epoch))
        torch.save(discriminator.state_dict(), "bkab1/%s/discriminator_%d.pth" % (dataset_name, epoch))
