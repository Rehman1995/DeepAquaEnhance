...
from utils.data_utils1 import Dataloader
#from op import FusedLeakyReLU, fused_leaky_relu, upfirdn2d, conv2d_gradfix
# print("Importing Done")

parser = argparse.ArgumentParser()
parser.add_argument("--cfg_file", type=str, default="configs/train_MuLA-GAN2.yaml")
parser.add_argument("--epoch", type=int, default=790, help="which epoch to start from")
parser.add_argument("--num_epochs", type=int, default=1100, help="number of epochs of training")
...
checkpoint_dir = os.path.join("LSUInew/", dataset_name)
...
# Check if there are saved weights to load
if os.path.exists("LSUI/%s/generator_790.pth" % dataset_name):
    generator.load_state_dict(torch.load("LSUI/%s/generator_790.pth" % dataset_name))
    print("Generator weights (epoch 790) loaded successfully.")

if os.path.exists("LSUI/%s/discriminator_790.pth" % dataset_name):
    discriminator.load_state_dict(torch.load("LSUI/%s/discriminator_790.pth" % dataset_name))
    print("Discriminator weights (epoch 790) loaded successfully.")
...
if torch.cuda.is_available():
    ...
    print(f"GPU {device_id} ACTIVATED")
...
# Optimizers
optimizer_G = torch.optim.AdamW(generator.parameters(), lr=lr_rate, betas=(lr_b1, lr_b2))
optimizer_D = torch.optim.AdamW(discriminator.parameters(), lr=lr_rate, betas=(lr_b1, lr_b2))


print("Begin Transforms")
...
for epoch in range(epoch, num_epochs):
    for i, batch in enumerate(dataloader):
        ...
        loss_G =  lambda_1 * loss_1  + lambda_con * loss_con   # loss_GAN +
        loss_G.backward()
        optimizer_G.step()
    ...
    if (epoch % ckpt_interval == 0):
        torch.save(generator.state_dict(), "LSUInew/%s/generator_%d.pth" % (dataset_name, epoch))
        torch.save(discriminator.state_dict(), "LSUInew/%s/discriminator_%d.pth" % (dataset_name, epoch))
