...
from models.Newbkab4_MuLA_GAN_Initial_model_1  import MuLA_GAN_Generator, Discriminator
...
parser.add_argument("--num_epochs", type=int, default=350, help="number of epochs of training")
...
checkpoint_dir = os.path.join("Newbkab4/", dataset_name)
...
for epoch in range(epoch, num_epochs):
    for i, batch in enumerate(dataloader):
        ...
        imgs_good_gt_r  = F.interpolate(imgs_good_gt, size=imgs_fake.shape[2:], mode='nearest')

        loss_1 = L1_G(imgs_fake, imgs_good_gt_r)
        loss_con = L_vgg(imgs_fake, imgs_good_gt_r)
    ...
    if (epoch % ckpt_interval == 0):
        torch.save(generator.state_dict(), "Newbkab4/%s/generator_%d.pth" % (dataset_name, epoch))
        torch.save(discriminator.state_dict(), "Newbkab4/%s/discriminator_%d.pth" % (dataset_name, epoch))
