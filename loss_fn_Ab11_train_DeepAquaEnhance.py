#ablation study 11:  loss fn :  loss_c  : b=3
...
checkpoint_dir = os.path.join("ab11/", dataset_name)
...
for epoch in range(epoch, num_epochs):
    for i, batch in enumerate(dataloader):
        ...
        loss_G =  lambda_con * loss_con    # loss_GAN + lambda_1 * loss_1  +
        loss_G.backward()
        optimizer_G.step()
    ...
    if (epoch % ckpt_interval == 0):
        torch.save(generator.state_dict(), "ab11/%s/generator_%d.pth" % (dataset_name, epoch))
        torch.save(discriminator.state_dict(), "ab11/%s/discriminator_%d.pth" % (dataset_name, epoch))
