...
parser.add_argument("--cfg_file", type=str, default="configs/EUVP_train_MuLA-GAN.yaml")
parser.add_argument("--epoch", type=int, default=270, help="which epoch to start from")
...
checkpoint_dir = os.path.join("EUVP_new/", dataset_name)
...
if os.path.exists("EUVP_new/%s/generator_270.pth" % dataset_name):
    generator.load_state_dict(torch.load("EUVP_new/%s/generator_270.pth" % dataset_name))
    print("Generator weights (epoch 270) loaded successfully.")

if os.path.exists("EUVP_new/%s/discriminator_270.pth" % dataset_name):
    discriminator.load_state_dict(torch.load("EUVP_new/%s/discriminator_270.pth" % dataset_name))
    print("Discriminator weights (epoch 270) loaded successfully.")
...
for epoch in range(epoch, num_epochs):
    for i, batch in enumerate(dataloader):
        ...
        loss_G =  lambda_1 * loss_1  + lambda_con * loss_con 
        loss_G.backward()
        optimizer_G.step()
    ...
    if (epoch % ckpt_interval == 0):
        torch.save(generator.state_dict(), "EUVP_new/%s/generator_%d.pth" % (dataset_name, epoch))
        torch.save(discriminator.state_dict(), "EUVP_new/%s/discriminator_%d.pth" % (dataset_name, epoch))
