...
from utils.UFO_data_utils import Dataloader
#from op import FusedLeakyReLU, fused_leaky_relu, upfirdn2d, conv2d_gradfix
# print("Importing Done")

parser = argparse.ArgumentParser()
parser.add_argument("--cfg_file", type=str, default="configs/UFO_train_MuLA-GAN.yaml")
...
checkpoint_dir = os.path.join("UFO_new/", dataset_name)
...
for epoch in range(epoch, num_epochs):
    ...
    if (epoch % ckpt_interval == 0):
        torch.save(generator.state_dict(), "UFO_new/%s/generator_%d.pth" % (dataset_name, epoch))
        torch.save(discriminator.state_dict(), "UFO_new/%s/discriminator_%d.pth" % (dataset_name, epoch))
